...
> **TL;DR**：投资 agent 偶尔能赢，但现有证据还不足以证明它能持续赚钱。几场真实资金实验的冠军不断更换，换个市场、换个时间窗口，排名就可能反转；放大到近百万持有者的 agent 金库，中位用户反而在亏损。眼下最稳定的商业模式不是让用户靠 agent 获得超额收益，而是向用户出售交易工具、投顾服务和课程——所以判断一个投资 agent，先看它能否公开长期、跨市场且扣除成本后的用户收益分布。
