...
进入正题前，先把四家的底牌摊开：四家里只有一家没有沙箱，而那一家把它写进了文档；另外三家都有某种 OS 级执行隔离，但覆盖范围各不相同，且各自都有已披露的漏洞。漏洞形态也五花八门，值得按机制分开看。
...
四家的安全承诺没有高下，只有取舍。OpenCode 把责任全部交回用户，好在它的文档把这一点写到明处；另外三家把隔离做进内核，好在——也坏在——每一层机制都藏着自己的破绽。而这一切的前提，是先有一份把边界写清楚的文档。