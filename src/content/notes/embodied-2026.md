...
slug: embodied-2026
description: 人形机器人已经跑赢人类半马纪录，但做饭、洗衣、叠衣仍停留在演示和试点。运动与操作是两类不同难度的问题：非结构化环境、柔性物体、长时程、低容错四难叠加，单步 99% 成功率连做 50 步只剩 60.5%。真正规模化的需求在工厂和仓库，不在家庭。
...
> **TL;DR**：截至 2026 年 8 月，机器人已跑赢人类半马纪录（50:26 对 56:42），但做饭、洗衣、洗碗、叠衣全部停留在演示或试点。「88% 家务失败率」是仿真 benchmark 的误读——真实数字是 BEHAVIOR Challenge 长时程家务约 12.4% 的完整任务成功率，真实家庭尚无足够部署样本。卡点不是运动而是操作：非结构化环境、柔性物体、长时程、低容错四难叠加，单步 99% 的成功率连做 50 步只剩 60.5%。真正量产的在工厂和仓库（Figure×BMW 装载 90,000 零件、Digit×GXO 搬运 100,000 周转箱、智元万台下线）；消费者能买到的最接近产品是 1X NEO，核心却是「机器人 + 云端远程人工」的 Expert Mode。判断成熟度别问「还有几年」，看六道门：可重复性、人工监督、续航、安全、经济性、规模。
